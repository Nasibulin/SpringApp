...
public interface CategoryRepository extends JpaRepository<Category, Integer> {
    ...
    @Query(value = "SELECT categories.id, categories.parent_id, catname FROM (SELECT CONNECT_BY_ROOT id AS parent_id, id FROM categories WHERE id = :catid CONNECT BY   NOCYCLE PRIOR id = parent_id) t1 JOIN categories ON t1.parent_id = categories.id WHERE categories.id > -50 ORDER BY parent_id",
           nativeQuery = true)
